...
public class LibraryService {
	...
	public static void showBookList() {
		List<Bookdata> bookdatas=BookRepository.getBookList();
	...
	public static void borrowBook(int bookdata_id,int member_id) {
		...
		if(res==0 || res==404) {
			...
			//왜 먼저했지?
			int result=LoanRepository.insertBorrowBook(bookdata_id,member_id);
			if (result==1) {
	...
	public static void returnBook(int bookdata_id,int member_id) {
		//bookdata_id가 대출상태인지 -> available가 0일 때 = 대출 상태)
		//0=> 대출중, 1=> 대출가능 , 404 => 불가
		int res=BookRepository.borrowBook(bookdata_id);
		if(res==0) {
